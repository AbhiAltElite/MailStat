...
pub fn perform(
    ...
    // Group target messages by folder path.
    let mut by_folder: HashMap<(i64, String), Vec<(i64, u32, i64)>> = HashMap::new();
    {
        let mut stmt = conn
            .prepare(
                "SELECT m.id, m.uid, m.size, f.id, f.path FROM messages m \
                 JOIN folders f ON f.id = m.folder_id WHERE m.id = ?1 AND m.account_id = ?2",
            )
            .map_err(|e| e.to_string())?;
        for id in message_ids {
            if let Ok((mid, uid, size, fid, fpath)) = stmt.query_row(params![id, account_id], |r| {
                Ok((
                    r.get::<_, i64>(0)?,
    ...
    if kind == "demo" {
        ...
        for ((_fid, fpath), msgs) in &by_folder {
            session.select(fpath).map_err(|e| e.to_string())?;
